...
% TODO: Put this in Recommendations
% One complication of using fuzzy \textit{c}-means in \textit{R} at the moment is the lack of packages that would be able to handle data in the widespread \texttt{Raster} class format directly. In addition, the \texttt{spfkm} function is implemented purely in \textit{R} at the moment, and without any built-in multithreading, which results in slower performance. However, these are all opportunities for improving the package in the future.
