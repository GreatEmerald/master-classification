...
While error statistics are useful as an objective summary metric of prediction accuracy, they are limited to the size of the validation set, which is very small compared to the total number of pixels in a tile (480 pixels out of 101.6 million). Therefore visual validation was performed by having each of the classification method predict the land cover proportions in each pixel of the whole study area (PROBA-V tile). The prediction accuracy was then assessed by comparing prediction results with each other as well as with fine resolution imagery from Google in QGIS. Comparisons were done by looking at three land cover classes at a time, which could be mapped into the red, green and blue colour channels. The range of colour values was set to be stretched from 0-100 (land cover proportion in percent) to 0-255 in order to better see the differences in proportions.
