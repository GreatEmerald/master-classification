...
\usepackage{subfig} % Grid of figures
...
\begin{sidewaysfigure}
  \centering
  \subfloat[Full tile, Google true colour]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/fulltile-google}
  } \hfill
  \subfloat[Full tile, random forest regression]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/fulltile-rf}
  }\hfill
  \subfloat[Full tile, neural networks]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/fulltile-nn}
  }\hfill
  \subfloat[Full tile, fuzzy \textit{c}-means]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/fulltile-cm}
  }\
  \subfloat[Riga, Google true colour]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/riga-google}
  }\hfill
  \subfloat[Riga, random forest regression]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/riga-rf}
  }\hfill
  \subfloat[Riga, neural networks]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/riga-nn}
  }\hfill
  \subfloat[Riga, fuzy \textit{c}-means]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/riga-cm}
  }\
\end{sidewaysfigure}
...
\begin{sidewaysfigure}
  \ContinuedFloat
  \centering
  \subfloat[Kar\v{s}uva, Google true colour]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/karsuva-google}
  } \hfill
  \subfloat[Kar\v{s}uva, random forest regression]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/karsuva-rf}
  }\hfill
  \subfloat[Kar\v{s}uva, neural networks]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/karsuva-nn}
  }\hfill
  \subfloat[Kar\v{s}uva, fuzzy \textit{c}-means]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/karsuva-cm}
  }\
  \subfloat[Curonian spit, Google true colour]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/kursiunerija-google}
  }\hfill
  \subfloat[Curonian spit, random forest regression]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/kursiunerija-rf}
  }\hfill
  \subfloat[Curonian spit, neural networks]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/kursiunerija-nn}
  }\hfill
  \subfloat[Curonian spit, fuzzy \textit{c}-means]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/kursiunerija-cm}
  }\
  \subfloat[Med\v{z}iokalnis, Google true colour]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/medziokalnis-google}
  } \hfill
  \subfloat[Med\v{z}iokalnis, random forest regression]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/medziokalnis-rf}
  }\hfill
  \subfloat[Med\v{z}iokalnis, neural networks]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/medziokalnis-nn}
  }\hfill
  \subfloat[Med\v{z}iokalnis, fuzzy \textit{c}-means]{
    \includegraphics[width=.23\textwidth]{../plot/figures-qgis/medziokalnis-cm}
    \label{subfig-medziokalnis-cm}
  }\
  \caption{Visual comparison of fuzzy classification prediction output. Red, green and blue colour channels represent: b-d and q-t: crops, deciduous trees, evergreen trees; f-h: urban, wetlands, water; j-l: urban, deciduous trees, evergreen trees; n-p: bare soil, grass, evergreen trees, respectively.}
  \label{fig-visualcomparison}
\end{sidewaysfigure}

To better visualise the differences between the fuzzy classification methods, predictions were made using each method for the whole study area. See figure \ref{fig-visualcomparison} for some examples. Overall visual comparison confirms that the differences between the different methods are rather minor. However, there are some small differences. %\protect\subref{subfig-medziokalnis-cm} shows that fuzzy \texit{c}-means tends to overestimate urban.

\section{Variable importance}
%TODO: Do I put in the wetlands curve?
%TODO: Describe random forest permutation importance specifics

Covariates that were important for classification varied depending on the class (see figure \ref{fig-variable-importance}). The covariate that had the most importance for all classes was mean NDVI, and both the red band and near infrared bands were important for predicting most classes. The water mask, aspect and \nth{1} order phase were the least important. Some of the covariates had high importance for just one class, such as \nth{2} order amplitude for crops, \nth{2} order phase for wetlands, terrain position index for shrubs. Three variables were confusing: \nth{2} order amplitude hindered shrub prediction, aspect hindered evergreen tree prediction and the water mask hindered wetland prediction.
...
%Conclusions:
%- Differences between methods are minor, once optimised; differences in variables are more important, and dropping variables can lead to increase in accuracy for some algorithms
%- Binary relevance method performs the best, which also allows for extra optimisation per-class, but takes more time; all can be parallelised, so the additional time cost may not necessarily be a problem, but nonetheless a factor to consider
%- If performance is a factor, c-means and neural networks give about the same speed and quality. Neural networks are slightly faster, whereas c-means needs fewer covariates for the same task.
%- Training on only pure pixels works, but validation and thus optimisation is difficult. Fuzzy ground truth data is quite important.
%- Fuzzy classification using the covariates works well for detecting mixed forests and wetlands, but more ground truth is needed for both training and validation to make sure
%- Some more covariates needed to discern water from urban and bare soil from peat; also covariates that help predict grass, shrubs and crops
%- More ground truth needed, better cloud masking, snow really helps in interpretation
...
%Another consideration for fuzzy \textit{c}-means is that its training is reduced to class means and their standard deviations, which limits its application to covariates that have a normal distribution.
...
From the covariates, most important was mean NDVI. It is a good indicator of vegetation properties and especially helps to separate vegetation classes from water, urban and bare soil. Slope and NIR were useful for classification of each class as well. Slope particularly helped to determine the class fraction of wetlands, which require flat terrain for water to stay, and water itself, since the surface of open water is always perfectly flat. NIR at summer particularly helped to determine crop and water fractions, as crops have very high reflectance in NIR during the peak of their growth, whereas water has low NIR throughout the year. LSWI, red and \nth{1} order amplitude helped to distinguish between all classes, with none standing out in particular. SWIR also heloped overall, but it was particularly useful for water.

Some covariates were useful for particular classes. \nth{2} order amplitude was useful for discerning crops, as they are harvested rather than allowed to grow naturally throughout the year. \nth{2} order phase was useful for discerning wetlands, as their NDVI curve tends to peak earlier in the year compared to other classes. TPI was most useful for shrubs, since their defining characteristic is height and they tend to grow in clumps further away from other types of vegetation.
