...
InputDir = "/userdata2/master-classification/dem/aster/"
...
AsterZips = list.files(InputDir, glob2rx("*.zip"), full.names = TRUE)

cl <- makeCluster(3, outfile="")
registerDoSNOW(cl)
pb = txtProgressBar(max = length(AsterZips), style = 3)

# Iterate over all zip files
foreach (AsterZip=AsterZips, .packages=c("raster"),
